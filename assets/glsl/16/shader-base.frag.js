...
void main () {
  ...
  float radius = 0.2;
  float smoothing = 0.1;
  float mask1 = smoothstep(radius, radius + smoothing, distance(center, st));
  float mask2 = step(0.4, st.x) + 1. - step(0.6, st.x) ;
    
  float mask = mix(mask1,mask2, 0.);
  //mask = fract(mask)*10.;
    
  color = mix(1.-coloring(lineB, vec3(st.x,st.y,0.2)), 1. - coloring(lineF, vec3(1.,0.,0.2)), mask);
